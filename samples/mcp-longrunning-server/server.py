...
async def run_blocking_operation(
    ...
    try:
        ...
        while elapsed < total_seconds:
            ...
            if enable_logs:
                await ctx.info(f"{operation_name}: {current_progress/total_seconds*100:.1f}% - {message}")
...
async def run_random_duration_task(
    ...
    result = await run_blocking_operation(
        int(random_duration), progress_interval_seconds, enable_logs, operation_name, ctx
    )
